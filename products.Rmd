...
p <- p + geom_histogram(alpha = 0.2,bins=30) + facet_grid(Product~REGION, scales = "free_y") + 
...
p <- ggplot(data=Main,aes(x=DATE,y=`M.Ton Price`,colour=Product)) + geom_jitter(alpha=0.1) + geom_smooth(method = "lm", formula = y ~ splines::bs(x, 3),se=FALSE)
p <- ggplotly(p)
hide_legend(p) %>% config(displaylogo = FALSE) %>%  config(displayModeBar = FALSE)
